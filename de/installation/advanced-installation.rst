...
Es mag Fälle geben in denen Du das CakePHP Verzeichnis an anderen Ortem im 
Dateisystem plazieren möchtest. Dies kann auf Grund von Shared Hosting 
Restriktionen sein oder möglicherweise möchtest Du einfach mit mehreren 
Anwendungen die CakePHP Bibliotheken teilen. Diese Sektion beschreibt wie Du 
CakePHP im Dateisystem plazieren kannst.

Realisiere zuerst diese drei Hauptteile einer Cake Anwendung.:

#. Die Kern CakePHP Biblitheken in /cake.
#. Dein Anwendungscode in /app.
#. Das Anwendungs-Webroot, üblicherweise in /app/webroot.

Jedes dieser Verzeichnisse kann sich überall in Deinem Dateisystem befinden, mit
der Ausnahme des webroot, welches für Deinen Webserver erreichbar sein muß. Du 
kannst sogar das webroot aus dem dem app Verzeichnis entfernen so lange Du Cake 
mitteilst wo es zu finden ist.

Um Deine CakePHP Installation zu konfigurieren mußt Du ein paar Änderungen in 
folgenden Dateien durchführen:
...
Hier sind drei Konstanten die Du ändern mußt: ``ROOT``,
...
Laß uns ein Beispiel durchgehen, so das Du siehst wie eine fortgeschrittene 
Installation in der Praxis aussieht. Stelle Dir vor das ich CakePHP wie wie 
folgend betreiben möchte:


-  Die CakePHP Kern-Bibliotheken werden in /usr/lib/cake plaziert.
-  Mein Anwendungs webroot wird /var/www/mysite/.
-  Mein Anwendungs verzeichnis wird /home/me/myapp.

Unter diesen Vorraussetzen muß ich die Datei webroot/index.php bearbeiten 
(welche hier sein wird /var/www/mysite/index.php in diesem Beispiel) um wie 
folgt auszusehen::
...
Es wird empfohlen die ``DS`` Konstante zu benutzen anstatt Slashes um 
Dateipfade abzuschließen. Das verhindert das Du fehlende Datei Error bekommst 
als ein Resultat des falschens Delimiters und macht Deinen Code portabler.


Apache und mod\_rewrite (und .htaccess)
=======================================
